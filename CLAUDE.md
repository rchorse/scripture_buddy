...
## Hard rules
- **No chat/messaging features. Ever.** Final product decision; the only user-visible UGC is custom display names (Haiku-checked, fail-closed).
- **No social features for under-13 accounts. Ever.** Final product decision, enforced in `app/services/friendships.py` — not a flag. A child is never visible to another user. This is also load-bearing for the COPPA consent method, so don't relax it without re-reading `docs/compliance/coppa-program.md`.
...
- Verifiable parental consent uses **email-plus** (one-time link to the parent's email + delayed confirmation). Consent is **per-scope** (`account`/`ai_processing` — there is no `social` scope) and independently revocable; `core.consent_audit` is append-only and survives account deletion. See `docs/compliance/coppa-program.md`.
