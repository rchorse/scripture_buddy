...
- Under-13 accounts **can** have friends and leaderboards — gated on the parent granting the `social` consent scope. "Social" never means messaging; there is no chat anywhere.
...
- Verifiable parental consent uses **email-plus** (one-time link to the parent's email + delayed confirmation). Consent is **per-scope** (`account`/`ai_processing`/`social`) and independently revocable; only `account` is required to use the app; `core.consent_audit` is append-only and survives account deletion. See `docs/compliance/coppa-program.md`.
